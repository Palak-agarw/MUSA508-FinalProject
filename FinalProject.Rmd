...
---
# Introduction
California has more wildfires than any other U.S. state. In 2020, California saw one of the worst wildfire seasons in recorded history, and climate change could lead to even more wildfire destruction in the future (Yan et al., 2020; Miller et al., 2020). The map below visualizes fires in California from the past 10 years. The spatial process of fires appears to be somewhat clustered.
...
```{r setup2, message=FALSE, warning=FALSE, cache=TRUE, results=FALSE}
# READ IN DATA
...
In California, homeowners in areas with high wildfire risk are required to take certain preventative measures, including clearing flammable vegetation around their home and keeping their roof clear of leaves (Public Resources Code, n.d.). Cal Fire, the state fireighting agency, is responsible for inspecting many properties within the state for compliance with these regulations. However, due to limited funding, Cal Fire is not able to complete their inspections. In 2018, Cal Fire inspected only 17% of the properties within their jursidiction. In Northern California, which includes some of the highest risk fire areas in the state, the inspection rates are even lower (Sommer, 2019).
...
SparkSafe will enable Cal Fire to dramatically expand their reach and help to improve wildfire prevention compliance throughout the 13 counties. The following counties are included in the app due to having both high wildfire risk and low Cal Fire inspection rates:
...
SparkSafe's wildfire risk score is based on the probability of a wildfire occuring in a given area as predicted by a logistic regression. This document outlines how SparkSafe's data scientists developed that logistic regression. In sharing this, we hope to inspire other agencies to open the "black box" of algorithmic governance and to increase transparency in the algorithms used to inform public policy decisions.

Follow this link to view a short video about the SparkSafe app: https://youtu.be/gXrzPI28f2Q

# Data
The following datasets were used to develop the logistic model. All data were obtained from publically available sources.
...
## ArcMap Processing
Many of the datasets were available in raster form. Because R does not support raster well, we did our initial data wrangling in ArcMap. First, we created a .5 x.5 mile fishnet for the entire state. We chose this size because .25 square miles is approximately the size of a residential neighborhood (Coulton et al., 2013).

![Creating Fishnet](C:/Users/owner160829a/Desktop/Graduate School/Penn/Courses/Fall 20/MUSA 508/Final Project/create_fishnet3.jpg)
...
![Reclassifying Data](C:/Users/owner160829a/Desktop/Graduate School/Penn/Courses/Fall 20/MUSA 508/Final Project/reclassify.jpg)

We then calculated slope from the elevation data using the slope spatial analyst tool.

![Calculating Slope](C:/Users/owner160829a/Desktop/Graduate School/Penn/Courses/Fall 20/MUSA 508/Final Project/slope.jpg)
...
![Zonal Statistics](C:/Users/owner160829a/Desktop/Graduate School/Penn/Courses/Fall 20/MUSA 508/Final Project/zonal.jpg)
...
![Join](C:/Users/owner160829a/Desktop/Graduate School/Penn/Courses/Fall 20/MUSA 508/Final Project/join.jpg)


```{r data, message=FALSE, warning=FALSE, cache=TRUE, results=FALSE}
## READ IN DATA
...
The distance variables were only calculated with grid cells located within the 13 counties. This may lead to misleading results. For example, the closest wildlife urban interface to a given grid cell could be outside of the 13 counties, perhaps even over the state border. Future versions of the SparkSafe model will include data outside of the selected counties for nearest neighbor calculations.  
...
+ The mean distance to grid cells that had fires in 2010-2013 is smaller for cells that had fires
...
+ Fires seem especially prevelant in savannas
...
## Goodness of Fit
### Pseudo R Squared
The McFadden pseudo r squared for this model is about .39. While this number is not particularly meaningful on its own, the McFadden pseudo r squared results for different iterations of the model were compared. A higher McFadden pseudo r squared indicates better accuracy. 
...
The receiver operating characteristic (ROC) curve visualizes trade-offs for different thresholds. As true positives in the model increase, the number of false positives also increase. For the SparkSafe app, we are more interested in increasing the true positive rate than in decreasing the false positive rate. However, if the false positive rate is too high, homeowners may not take the app's predictions seriously. This could lead to a decrease in compliance with wildfire prevention measures in high fire risk areas.
...
Given the distribution of predicted probabilities for grid cells that have fires, the default threshold of .5 does not make sense. We would prefer to over predict fires with the SparkSafe app than to underpredict. 
...
To choose a threshold, we replaced the 2014-2018 weather data with 2019 weather data in our model and generated probability predictions. We then visualized the distribution of predicted probabilities for cells with and without fire in 2019 and calculated the mean predicted probability for both cases. 

```{r 2019, message=FALSE, warning=FALSE, cache=TRUE}
...
hist(testProbs19fire$Probs, 
     col="#F96167",
       main="Predicted Probabilities for Grid Cells with Fire in 2019",
     xlab="Probability")

hist(testProbs19nofire$Probs, 
     col="#FCE77D",
       main="Predicted Probabilities for Grid Cells with No Fire in 2019",
     xlab="Probability")
```

The mean predicted probability for cells with no fire is 0.006. The mean predicted probability for cells with fire is .248. This led us to choose a threshold of .2.

```{r 2019means, message=FALSE, warning=FALSE, cache=TRUE}
##0.006063904
...
The confusion matrix below shows that our sensitivity is poor when predicting on 2019 data. This may be because our model is overfit on the training dataset and not generalizable to future years. In our next SparkSafe update, we aim to improve the ability of the model to forecast fires in future years.

```{r 2019matrix, message=FALSE, warning=FALSE, cache=TRUE}
...
## Cross Validation
Although our  model did not predict well on 2019 data, we performed other validation tests to judge the generalizability of our model. First, cross validation with 100 k folds was performed. The graphs below visualize the area under the ROC curve, sensitivity, and specificity of the final model across folds. Each of these plots are clustered around the mean, indicating our model has decent generalizability. However, the generalizability of the model's sensitivity could be improved.
...
cvFit <- train(Fire1418 ~ ., data = fishnet_clipped %>% st_drop_geometry() %>%
                 dplyr::select(
                   -ID,-Fire19,-weather_ID, -WUI_MAJ,-WUI.nn,
                   -MeanMaxTemp,-FIRE, -CoverCat, -ElevationBi,
                   -Fire.nn, -SlopeCat, -MeanPrecip1418, -Max_Temp19, 
                   -Mean_Temp19, -Mean_Precipitation19, 
                   -Mean_Humidity19, -Mean_Wind_Speed19, -Fire1013, -Shrub.nn)%>%
...
The map below visualizes actual fires and predicted probabilities for each of the 13 counties. This shows that the model seems to correctly predict actual fires equally well across the counties. However, some counties have more areas with high predicted probability of fire where no fire actually occured. This suggests that more work could be done to improve the generalizability of the model across space.
...
fire1418_clipped <- st_intersection(st_make_valid(fire_perimeter1418),st_make_valid(selected_counties))

ggplot() +
  geom_sf(data = reg.spatialcv, aes(fill = Prediction), color = "transparent")+
  geom_sf(data = fire1418_clipped, fill = "transparent", color = "red", size=.5)+
...
# Discussion
Given the difficulty of predicting for rare events, and the notorious difficulty of forecasting wildfire behavior in particular, we are pleased with the performance of our model (Massey, 2013). Our cross validation indicates that the model is generalizable to new data, though the generalizability across different counties could be improved. At a .2 threshold, which we feel is reasonable given the costs and benefits of overpredicting versus underpredicting fire, the model has high sensitivity and specificity. Each of the independent variables in the model is highly statistically significant.

While the model can predict fire over the course of several years with reasonable accuracy, it does not perform as well when predicting on a single year, as we saw when predicting for 2019 fires. This is likely because fire is a relatively rare event over the course of a single year, even though wildfires have become an increasing threat in California. The model may also be overfit on the training data.
...
Rather than predicting whether or not fire will occur in a given grid cell, SparkSafe provides users with a risk score of Low, Medium, or High. This helps to address the issue of false negatives in our model, as homes with predicted fire probabilities below the .2 threshold but above .03 will be categorized under Medium Risk.

# Conclusion
The SparkSafe model offers a promising means of forecasting wildfire risk in Northern California. As we continue to refine the model, we aim to achieve the following:

+ Increase the sensitivity of the model while keeping the false negative rate reasonably low

+ Improve the model's generalizability across space

+ Incorporate accurate weather forecasts

+ Expand the spatial scale of nearest neighbor feature calculations to include data outside of the 13 counties

+ Generate wildfire risk predictions at a smaller spatial scale

While there is room for improvement, we believe that the current version of the model provides useful insights into wildfire risk in Northern California. Using our app, homeowners can enter their address and instantly get a fire risk prediction. Homeowners in high risk areas will be prompted to submit a remote inspection to Cal Fire. We anticipate that this will improve compliance with fire prevention regulations across Northern California. 
...
Coulton, C.J., Jennings, M.Z. & Chan, T. How Big is My Neighborhood? Individual and Contextual Effects on Perceptions of Neighborhood Scale. *Am J Community Psychol* 51, 140–150 (2013). https://doi.org/10.1007/s10464-012-9550-6
